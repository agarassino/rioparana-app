...
export const STATIONS_BY_REGION = {
  'Alto Paraná': STATIONS.filter(s => ['libertad', 'eldorado', 'santa-ana', 'posadas', 'ituzaingo', 'ita-ibate', 'itati', 'paso-de-la-patria'].includes(s.id)),
  'Paraná Medio': STATIONS.filter(s => ['corrientes', 'barranqueras', 'empedrado', 'bella-vista', 'goya', 'reconquista', 'esquina', 'la-paz', 'santa-elena', 'hernandarias', 'parana', 'santa-fe', 'diamante'].includes(s.id)),
  'Paraná Inferior': STATIONS.filter(s => ['victoria', 'san-lorenzo', 'rosario', 'villa-constitucion', 'san-nicolas', 'ramallo'].includes(s.id)),
  'Delta': STATIONS.filter(s => ['san-pedro', 'baradero', 'zarate', 'campana', 'escobar', 'martin-garcia', 'dique-lujan', 'tigre', 'san-fernando', 'san-isidro', 'olivos'].includes(s.id)),
};
